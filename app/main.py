...
def update_advertisement(advertisement_id: int, user_id: int, advertisement_in: advertisement_schema.AdvertisementUpdate, db: Session = Depends(get_db)):
    db_advertisement = advertisement_functions.get_advertisement(db, advertisement_id=advertisement_id, owner_id=user_id)
    if db_advertisement is None:
        raise HTTPException(status_code=404, detail="Advertisement not found")

    return advertisement_functions.update_advertisement(db, db_advertisement=db_advertisement, owner_id=user_id, advertisement_in=advertisement_in)


@app.put("/users/{user_id}/drafts/{draft_id}", response_model=advertisement_schema.Advertisement)
def update_draft(draft_id: int, user_id: int, draft_in: advertisement_schema.AdvertisementUpdate, db: Session = Depends(get_db)):
