...
def create_item_for_user(
    ...
    return user_crud.create_user_item(db=db, news=news, user_id=user_id)


@app.get("/items/", response_model=list[user_schema.News])
def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    news = user_crud.get_news(db, skip=skip, limit=limit)
    return news
...
def update_user(user_id: int, user: user_schema.UserUpdate, db: Session = Depends(get_db)):
    db_user = user_crud.get_user_by_email(db, email=user.email)
    return user_crud.update_user(db=db, user=user, user_id=user_id)
